...
function App() {
                    ...
                    if (existingCasesMap.has(currentSN)) {
                        ...
                        try {
                            const [analysis, priority, sentiment] = await Promise.all([
                                getAIAnalysisAndCategory(row), getAIPriority(row['obs']), getAISentiment(row['obs'])
                            ]);
                            aiAnalysisCat = analysis; aiPrio = priority; aiSentiment = sentiment; relNum = extractRelatedComplaintNumber(row['obs']);
    ...
    async function handleReporteCruceUpload(event) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                const reportNuips = new Set(reportData.map(row => normalizeNuip(row[nuipHeader])).filter(nuip => nuip));
    ...
    async function handleOpenCaseDetails(caseItem) {
        ...
        const normalizedCaseNuips = new Set([normalizeNuip(caseItem.Nro_Nuip_Cliente), normalizeNuip(caseItem.Nro_Nuip_Reclamante)].filter(nuip => nuip && nuip !== '0' && nuip !== 'N/A'));
        cases.forEach(otherCase => {
            if (otherCase.id === caseItem.id) return;
            const normalizedOtherNuips = new Set([normalizeNuip(otherCase.Nro_Nuip_Cliente), normalizeNuip(otherCase.Nro_Nuip_Reclamante)].filter(Boolean));
        ...
        if (reporteCruceData.length > 0 && reporteCruceData[0]) {
            ...
            if (nuipColumns.length > 0 && snHeader) {
                reporteCruceData.forEach((reportRow, index) => {
                    ...
                    if (!reportSN) return;
                    const reportRowNuips = new Set(nuipColumns.map(col => normalizeNuip(reportRow[col])).filter(Boolean));
    ...
    async function handleManualSubmit(e) {
        ...
        try {
            ...
            try {
                const [analysis, priority, sentiment] = await Promise.all([
                    getAIAnalysisAndCategory(aiData), getAIPriority(manualFormData.OBS), getAISentiment(manualFormData.OBS)
                ]);
                aiAnalysisCat = analysis; aiPrio = priority; aiSentiment = sentiment; relNum = extractRelatedComplaintNumber(manualFormData.OBS);
    ...
    async function handleMassUpdate() {
        ...
        try {
            ...
            docsToUpdateSnapshot.docs.forEach(docSnapshot => {
                ...
                else if (massUpdateTargetStatus === 'Resuelto') { updateData['Fecha Cierre'] = today; updateData.Tiempo_Resolucion_Minutos = currentCase.Fecha_Inicio_Gestion ? getDurationInMinutes(currentCase.Fecha_Inicio_Gestion, nowISO) : 'N/A'; }
            ...
            if (user) {
                ...
                try {
                    ...
                    if (initialAuthToken && (error.code === 'auth/invalid-custom-token' || error.code === 'auth/invalid-claims')) {
                        ...
                        catch (anonError) { console.error("CRITICAL: Fallback anonymous sign-in also failed.", anonError); displayModalMessage(`Error de Autenticación Crítico: ${anonError.message}`); setLoading(false); }
                    } else { console.error("CRITICAL: Authentication has failed completely.", error); displayModalMessage(`Error de Autenticación Crítico: ${error.message}`); setLoading(false); }
                }
            }
        });
                    ...
                    <div className="my-6 p-4 border border-blue-300 bg-blue-50 rounded-lg shadow-md">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            <select value={massUpdateTargetStatus} onChange={(e) => setMassUpdateTargetStatus(e.target.value)} className="p-2 border rounded-md shadow-sm flex-grow">
                                ...
                                {ALL_STATUS_OPTIONS.map(status => (<option key={status} value={status}>{status}</option>))}
                            </select>
                    ...
                    <div className="bg-white rounded-lg shadow-xl p-6 max-w-4xl w-full mx-auto overflow-y-auto max-h-[90vh]">
                        ...
                        {selectedCase.Estado_Gestion === 'Escalado' && (<div className="mt-4 mb-6 p-4 border border-red-200 rounded-md bg-red-50">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div><label htmlFor="areaEscalada" className="block text-sm font-medium text-gray-700 mb-1">Área Escalada:</label><select id="areaEscalada" name="areaEscalada" value={selectedCase.areaEscalada || ''} onChange={(e) => handleModalFieldChange('areaEscalada', e.target.value)} className="block w-full input-form"><option value="">Seleccione Área...</option>{AREAS_ESCALAMIENTO.map(area => <option key={area} value={area}>{area}</option>)}</select></div>
                                <div><label htmlFor="motivoEscalado" className="block text-sm font-medium text-gray-700 mb-1">Motivo/Acción Escalado:</label><select id="motivoEscalado" name="motivoEscalado" value={selectedCase.motivoEscalado || ''} onChange={(e) => handleModalFieldChange('motivoEscalado', e.target.value)} className="block w-full input-form" disabled={!selectedCase.areaEscalada}><option value="">Seleccione Motivo/Acción...</option>{(MOTIVOS_ESCALAMIENTO_POR_AREA[selectedCase.areaEscalada] || []).map(motivo => <option key={motivo} value={motivo}>{motivo}</option>)}</select></div>
